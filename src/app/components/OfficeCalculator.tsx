...
export default function OfficeCalculator({ onOrder }: { onOrder?: (params:any)=>void }) {
  const [size,setSize]=useState(0), [floor,setFloor]=useState(1), [urgent,setUrgent]=useState(false);
  const calcPrice=()=>{let p=100*(size||1); if(floor>5)p*=1.1; if(urgent)p*=1.2; return Math.round(p);};
  ...
  return (<div onClick={e=>e.stopPropagation()} className="p-2 bg-white rounded-lg shadow-inner space-y-2 text-sm">
    ...
    <button onClick={()=>onOrder?.({size,floor,urgent,price:calcPrice()})} className="w-full mt-1 bg-green-500 text-white rounded-full py-1 hover:bg-green-600">Заказать услугу</button>
  </div>);
}
