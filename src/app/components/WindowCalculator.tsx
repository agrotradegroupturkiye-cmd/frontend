...
export default function WindowCalculator(){
  ...
  const createOrderAndRedirect = () => {
    const order = {
      ...
      service: 'Мойка окон',
      params: { count, size, floor, urgent },
  ...
  return (
    <div onClick={e=>e.stopPropagation()} className="p-4 bg-white rounded-lg shadow-inner space-y-4 w-full">
      <label className="block">Количество окон</label>{renderCounter(count,setCount)}
      <label className="block">Площадь одного окна (м²)</label>
